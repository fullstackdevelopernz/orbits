...
function collectionPreview(key, context='card'){
  ...
  return `<div class="collection-preview collection-preview-${context} ${key}">
    ...
    <span class="collection-brand-sticker"><img src="assets/branding/orbits-logo.png" alt="ORBITS"></span>
  </div>`;
}
function heroOrbit(src,alt,className){ return `<a class="hero-orbit ${className}" href="#shop"><img src="${src}" alt="${alt}"></a>`; }
...
function homeTemplate(){
  ...
  <section class="hero hero-v2">
    <div class="hero-copy">
      ...
      <h1>Little plush personalities with big main-character energy.</h1>
      <p class="hero-intro">Clip on your mood, your sign or your intention. Every Orbit is soft, expressive, giftable and made to turn an everyday bag into your bag.</p>
      <div class="hero-actions">
        <a class="primary" href="#shop">Shop all Orbits</a>
        <a class="secondary" href="#collection/zodiac">Meet the collections</a>
      </div>
      <div class="hero-proof"><span>36 collectible Orbits</span><span>3 themed worlds</span><span>10–12 cm plush charms</span></div>
    </div>
    <div class="hero-stage" aria-label="Featured Orbit plush bag charms">
      ...
      <div class="hero-stage-badge">Collect your world</div>
    </div>
  </section>
  <section class="section">
    <div class="section-heading"><div><p class="eyebrow">Three worlds. Pick yours.</p><h2>Choose your Orbit collection</h2></div><p>The pop-art energy changes with each collection, but the Eden Toy Co. design language stays unmistakably ORBITS.</p></div>
    <div class="collection-grid">
      ${collectionCard('crystal')}${collectionCard('manifestation')}${collectionCard('zodiac')}
    </div>
  </section>
  <section class="shop-shell">
    <div class="section-heading"><div><p class="eyebrow">Fan favourites</p><h2>Start your collection</h2></div><a class="secondary" href="#shop">See all 36</a></div>
...
function collectionTemplate(key,items){
  const meta = collectionMeta(key);
  return `<section class="collection-hero collection-hero-brand ${key}"><div><p class="eyebrow">ORBITS · 12 collectible charms</p><h1>${collectionTitle(key)}</h1><p>${collectionCopy(key)}</p><a class="primary" href="#shop">Shop all Orbits</a></div>${collectionPreview(key,'detail')}</section><section class="shop-shell"><div class="section-heading"><div><p class="eyebrow">${meta.kicker}</p><h2>Collect all 12</h2></div><p>${meta.copy}</p></div>${productGrid(items)}</section>`;
}
...
function bagTemplate(){
  ...
  if(!entries.length) return `<section class="page-shell"><div class="page-panel"><span class="page-kicker">Your bag</span><h1>Nothing clipped in yet.</h1><p class="muted">Pick an Orbit and start your collection.</p><a class="primary" href="#shop">Shop Orbits</a></div></section>`;
  return `<section class="page-shell"><div class="page-panel"><span class="page-kicker">Your bag</span><h1>Your Orbit lineup.</h1><div>${entries.map(({p,qty})=>`<div class="order-row"><img src="${p.image}" alt="${p.name}"><div><strong>${p.name}</strong><div class="qty"><button data-bagqty="${p.id}" data-delta="-1">−</button><span>${qty}</span><button data-bagqty="${p.id}" data-delta="1">+</button></div></div><strong>${money(p.price*qty)}</strong></div>`).join('')}</div><div class="order-total"><span>Subtotal</span><span>${money(cartSubtotal())}</span></div><a class="primary wide" href="#checkout">Proceed to checkout</a></div></section>`;
}
function checkoutTemplate(){
  const entries = cartEntries();
  if(!entries.length) return bagTemplate();
  return `<section class="page-shell"><div class="checkout-grid"><form class="checkout-card" id="checkoutForm"><span class="page-kicker">Checkout</span><h1>Almost yours.</h1><div class="checkout-step"><h2>Contact</h2><div class="form-grid"><div class="form-field full"><label>Email</label><input required type="email" placeholder="you@example.com"></div></div></div><div class="checkout-step"><h2>Delivery</h2><div class="form-grid"><div class="form-field"><label>First name</label><input required></div><div class="form-field"><label>Last name</label><input required></div><div class="form-field full"><label>Address</label><input required></div><div class="form-field"><label>City</label><input required></div><div class="form-field"><label>Postcode</label><input required></div><div class="form-field full"><label>Country</label><select><option>New Zealand</option></select></div></div></div><div class="checkout-step"><h2>Payment</h2><p class="secure-note">Secure-payment styling preview. A live payment gateway has not been connected, so no card details are collected and no charge will occur.</p></div><button class="primary wide" type="submit">Complete demo checkout</button></form><aside class="order-summary"><h2>Order summary</h2>${entries.map(({p,qty})=>`<div class="order-row"><img src="${p.image}" alt="${p.name}"><div><strong>${p.name}</strong><small>Qty ${qty}</small></div><strong>${money(p.price*qty)}</strong></div>`).join('')}<div class="order-total"><span>Total</span><span>${money(cartSubtotal())} NZD</span></div></aside></div></section>`;
}
function confirmationTemplate(){ return `<section class="page-shell"><div class="page-panel account-card"><span class="page-kicker">Demo checkout complete</span><h1>Your Orbits are looking good.</h1><p class="muted">This is the confirmation-state design only. No order was submitted and no payment was taken.</p><a class="primary" href="#shop">Keep shopping</a></div></section>`; }
