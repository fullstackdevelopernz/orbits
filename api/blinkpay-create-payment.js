const {
  ...
  supabaseRest, resolveCustomer, value2, json, allowedMethod, bearer
}=require('./_blinkpay');
...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    const reference=String(edenOrder.order_number||edenOrder.id).replace(/[^A-Za-z0-9]/g,'').slice(-12) || 'ORBITS';
    const qp=await client.createQuickPayment({
      flow:{detail:{type:AuthFlowDetailTypeEnum.Gateway,redirectUri:'https://www.edentoyco.com/?blinkpay=returned'}},
      amount:{currency:AmountCurrencyEnum.NZD,total:value2(total)},
      pcr:{particulars:'ORBITS',code:'ORDER',reference}
    });
    if(!qp?.quickPaymentId || !qp?.redirectUri) throw new Error('BlinkPay did not return a payment redirect.');

    await supabaseRest('eden_payments',{method:'POST',body:{
      order_id:edenOrder.id,provider:'blinkpay',provider_reference:qp.quickPaymentId,status:'pending',amount:total,currency:'NZD',raw_payload:{quick_payment_id:qp.quickPaymentId,created:true}
    }});

    return json(res,200,{edenOrderId:edenOrder.id,orderNumber:edenOrder.order_number,quickPaymentId:qp.quickPaymentId,redirectUrl:qp.redirectUri});
  }catch(error){
    console.error('blinkpay-create-payment',error);
    return json(res,error.statusCode||500,{error:error.message||'Unable to start BlinkPay checkout.'});
  }
};
