(function(){
  ...
  checkoutTemplate = function(){
    ...
    return `<section class="page-shell paypal-checkout-shell"><div class="checkout-grid"><form class="checkout-card" id="checkoutForm">
      ...
      <p class="paypal-checkout-intro">Your Eden Toy Co order is created first, then you complete payment securely with PayPal.</p>
      ...
      <div class="checkout-step paypal-secure-step"><div><strong>Pay securely with PayPal</strong><p>You can use your PayPal account or the payment methods PayPal makes available at checkout.</p></div><span class="paypal-lock">Secure</span></div>
      <button class="primary wide paypal-checkout-button" type="submit">Continue to PayPal</button>
      <p class="fineprint">Your order is not marked paid until Eden Toy Co receives a successful PayPal capture confirmation.</p>
    </form><aside class="order-summary"><span class="page-kicker">Your preorder</span><h2>Order summary</h2>${entries.map(({p,qty})=>`<div class="order-row"><img src="${p.image}" alt="${esc(p.name)}"><div><strong>${esc(p.name)}</strong><small>Qty ${qty}</small></div><strong>${money(p.price*qty)}</strong></div>`).join('')}<div class="order-total"><span>Total</span><span>${money(cartSubtotal())} NZD</span></div><div class="paypal-summary-note">36 products. One secure checkout. Your order will appear in Eden Toy Co operations after it is created.</div></aside></div></section>`;
  };

  confirmationTemplate = function(){
    const last=JSON.parse(sessionStorage.getItem('eden-last-paid-order')||'null');
    return `<section class="page-shell"><div class="page-panel account-card eden-confirmation-card"><span class="page-kicker">Payment confirmed</span><div class="confirmation-check">✓</div><h1>Your Orbits are officially on order.</h1><p>${last?.orderNumber?`Order <strong>#${esc(last.orderNumber)}</strong> has been paid and recorded by Eden Toy Co.`:'Your PayPal payment has been captured and your Eden Toy Co order is recorded.'}</p><p class="muted">Keep an eye on your email for order and shipping updates.</p><div class="confirmation-actions"><a class="primary" href="#shop">Keep exploring</a><a class="secondary" href="#account">View account</a></div></div></section>`;
  };
  ...
  bindCheckout = function(){
    ...
    form.addEventListener('submit',async e=>{
      ...
      try{
        ...
        const response=await fetch('/api/paypal-create-order',{method:'POST',headers:{'Content-Type':'application/json',...(accessToken?{Authorization:`Bearer ${accessToken}`}:{})},body:JSON.stringify({
        ...
        setMessage(err.message||'Unable to start PayPal checkout.','error');
        button.disabled=false; button.textContent='Continue to PayPal';
      }
    });
  };
  ...
  async function processPayPalReturn(){
    ...
    if(!paypalOrderId){ location.hash='bag'; return; }
    ...
    try{
      ...
      sessionStorage.setItem('eden-last-paid-order',JSON.stringify({orderNumber:data.orderNumber,orderId:data.orderId,paypalOrderId}));
      sessionStorage.removeItem('eden-pending-paypal-order');
  ...
  window.addEventListener('DOMContentLoaded',()=>setTimeout(processPayPalReturn,50));
})();
